...
def main():
    ...
    with st.sidebar:
        ...
        with st.form("controls", clear_on_submit=False):
            ...
            **Commodities:** GC=F (Gold), CL=F (Oil)
            ...
            stress_thr = st.slider("Stress Threshold", 0.5, 5.0, 1.5, 0.1)
            extreme_thr = st.slider("Extreme Threshold", 1.0, 8.0, 2.5, 0.1)
    ...
    if run:
        try:
            df = load_and_compute(ticker, period, interval)

            df = df.copy()
            df["regime"] = "Normal"
            df.loc[df["stress_score"] > stress_thr, "regime"] = "Stress"
            df.loc[df["stress_score"] > extreme_thr, "regime"] = "Extreme"

            st.session_state["df"] = df
            st.session_state["ticker"] = ticker
            st.session_state["stress_thr"] = stress_thr
            st.session_state["extreme_thr"] = extreme_thr
